...
def get_locale():
    ...
    """
    loc_ = request.args.get('locale')
    if loc_ in app.config['LANGUAGES']:
        return loc_
    if g.user:
        loc_ = g.user.get('locale')
        if loc_ and loc_ in app.config['LANGUAGES']:
            return loc_
    loc_ = request.headers.get('locale', None)
    if loc_ in app.config['LANGUAGES']:
        return loc_
